...
MySqlSupplierRepository sqlSupplierRepo = new MySqlSupplierRepository(MYSQL_CONNETION_STRING);
...
void ManageSupplier()
{
    while (true)
    {
        Console.WriteLine("歡迎使用供應商管理介面");
        Console.WriteLine("請問您想使用什麼功能呢");
...
void DisplayMenu()
    {
        ...
        Console.WriteLine("6. 查詢低庫存的產品");
        Console.WriteLine("7. 查詢已缺貨的產品");
        Console.WriteLine("8. 調整產品數量作業");
...
//之後可以嘗試針對指定商品來提出改變
    void SearchLowStock()
    {
        Console.WriteLine("查詢低庫存的產品");
        List<Product> products = inventoryService.SearchLowStock();
        if (products.Any())
        {
            Console.WriteLine("-----------查詢條件為:(低庫存<小於10>)------------");
            Console.WriteLine("-----------------------------------------------");
            Console.WriteLine("ID | Name | Price | Quantity | Status");
            Console.WriteLine("-----------------------------------------------");
            foreach (var product in products)
    ...
    void SearchOutOfStockProduct()
    {
        ...
        if (products.Any())
        {
            Console.WriteLine("---------查詢條件為:已缺貨商品如下---------");
            Console.WriteLine("-----------------------------------------------");
            Console.WriteLine("ID | Name | Price | Quantity | Status");
            Console.WriteLine("-----------------------------------------------");
            foreach (var product in products)
